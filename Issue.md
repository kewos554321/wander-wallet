...
| # | 類別 | 描述 | 難度 |
|---|------|------|------|
| R5 | Refactor | 集中類型定義到 `types/index.ts` | 低 |

**P1 - 高優先**
| # | 類別 | 描述 | 難度 |
|---|------|------|------|
| U5 | UI/UX | Skeleton Loading 取代純文字載入狀態 | 低 |
| U6 | UI/UX | Toast 通知 (新增/編輯/刪除成功後) | 低 |

**P2 - 中優先**
| # | 類別 | 描述 | 難度 |
|---|------|------|------|
| 1 | 需求 | 串接 AI 模型自動轉換文本為支出紀錄 | 高 |
| R6 | Refactor | 建立格式化工具 `lib/formatters.ts` | 低 |
| R7 | Refactor | 建立開發模式配置 `lib/dev-mode.ts` | 低 |
| R8 | Refactor | 拆分 ExpenseForm 為子組件 | 中 |
| U1 | UI/UX | 下拉刷新 (Pull to refresh) | 低 |
| U3 | UI/UX | 日期分組顯示 | 低 |
| U12 | UI/UX | 長按支出卡片進入多選模式 | 低 |
| U13 | UI/UX | 快速金額按鈕 | 低 |
| U14 | UI/UX | 搜尋/篩選支出功能 | 中 |
| U15 | UI/UX | 支出列表排序 | 低 |
| S1 | 統計 | 時間篩選器（本週/本月/全部） | 中 |

**P3 - 低優先**
| # | 類別 | 描述 | 難度 |
|---|------|------|------|
| 5 | 需求 | 支出增加地點 (Google Maps/Places) | 中高 |
| R9 | Refactor | 引入 React Query/SWR 資料快取 | 高 |
| U2 | UI/UX | 無限滾動分頁 | 中 |
| U4 | UI/UX | 空狀態頁面插圖動畫 | 低 |
| U7 | UI/UX | Swipe to Delete 手勢 | 中 |
| U8 | UI/UX | 金額數字滾動動畫 | 低 |
| U16 | UI/UX | 頭像 tooltip | 低 |
| U17 | UI/UX | 高金額警示樣式 | 低 |
| U18 | UI/UX | 字數限制提示 | 低 |
| U19 | UI/UX | 深色模式過渡動畫 | 低 |
| S5 | 統計 | 個人消費排行 | 低 |
| S6 | 統計 | 付款者統計明細 | 低 |
...
| 3 | 支出可以增加消費清單圖片上傳功能 | 中高 | P2 | 已完成 | 2025-12-18 |
...
### #3 圖片上傳 (中高) ✅ 已完成
- 採用 base64 存儲方案 (與封面圖片相同做法)
- DB schema 加 `image` 欄位 (Text 類型)
- 前端 Canvas API 壓縮圖片 (最大 1200x1200，JPEG 80% 品質)
- 支援 5MB 以內圖片上傳
- 支出列表可點擊圖片圖標查看大圖
...
| # | 描述 | 難度 | 優先級 | 狀態 | 日期 |
|---|------|------|--------|------|------|
| R1 | 建立 API 認證中間件 `lib/api-middleware.ts`，統一 `getAuthUser` 和成員權限檢查 | 中 | P1 | 已完成 | 2025-12-18 |
| R2 | 建立統一 API 回應格式 `lib/api-response.ts`，標準化 success/error 結構 | 中 | P1 | 已完成 | 2025-12-18 |
| R3 | 建立統一驗證層 `lib/validators.ts`，集中金額、日期、參與者等驗證邏輯 | 中 | P1 | 已完成 | 2025-12-18 |
| R4 | 建立統一錯誤處理 `lib/error-handler.ts`，處理 Prisma 錯誤碼 (P2002/P2003/P2025) | 低 | P2 | 已完成 | 2025-12-18 |
...
| # | 描述 | 難度 | 優先級 | 狀態 | 日期 |
|---|------|------|--------|------|------|
| U9 | 支出摘要加入類別圓餅圖/長條圖 | 中 | P2 | 已完成 | 2025-12-18 |
| U10 | Project 頁面加入支出趨勢折線圖 (每日/每週) | 中 | P3 | 已完成 | 2025-12-18 |
| U11 | 成員貢獻度排行榜視覺化顯示 | 低 | P3 | 已完成 | 2025-12-18 |

### 統計頁面優化

| # | 描述 | 難度 | 優先級 | 狀態 | 日期 |
|---|------|------|--------|------|------|
| S1 | 時間篩選器：切換「本週/本月/全部」檢視不同時段統計 | 中 | P2 | 待處理 | 2025-12-18 |
| S2 | 分類百分比：圓餅圖旁顯示各類別佔比 % | 低 | P2 | 已完成 | 2025-12-18 |
| S3 | 最高單筆支出：顯示金額最大的那筆消費詳情 | 低 | P2 | 已完成 | 2025-12-18 |
| S4 | 每日平均支出：計算平均每天花費金額 | 低 | P2 | 已完成 | 2025-12-18 |
| S5 | 個人消費排行：誰花最多/誰付最多排行榜 | 低 | P3 | 已完成 | 2025-12-18 |
| S6 | 付款者統計：各成員付款次數與總額明細 | 低 | P3 | 待處理 | 2025-12-18 |
| S7 | 結算預覽卡片：快速顯示誰欠誰多少，連結到結算頁 | 低 | P2 | 已完成 | 2025-12-18 |
| S8 | 付款排行榜：顯示誰付最多錢（貢獻度排行） | 低 | P2 | 已完成 | 2025-12-18 |
| S9 | 消費排行榜：顯示誰花最多（應付金額排行） | 低 | P2 | 已完成 | 2025-12-18 |
| S11 | 類別趨勢圖：各類別隨時間變化（堆疊面積圖） | 中 | P2 | 已完成 | 2025-12-18 |
| S12 | 平均單筆消費：顯示平均每筆消費金額 | 低 | P2 | 已完成 | 2025-12-18 |
