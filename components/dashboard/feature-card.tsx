...
export function FeatureCard({ href, onClick, icon, label, iconBgClass, "data-tour": dataTour }: FeatureCardProps) {
  const content = (
  ...
  if (href) {
    return <Link href={href} data-tour={dataTour}>{content}</Link>
  }

  return (
    <button onClick={onClick} className="w-full text-left" data-tour={dataTour}>
      {content}
    </button>
  )
}
